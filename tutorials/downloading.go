...
func makeDownloadingTab(w fyne.Window) fyne.CanvasObject {
	table := widget.NewTableWithHeaders(
		...
		func() fyne.CanvasObject {
			return container.NewStack(widget.NewLabel("name"), widget.NewLabel("speed"), widget.NewProgressBar(), widget.NewLabel("total"), widget.NewButton("remove", func() {}))
		},
		func(id widget.TableCellID, cell fyne.CanvasObject) {
			...
			totalLabel := cell.(*fyne.Container).Objects[3].(*widget.Label)
			b := cell.(*fyne.Container).Objects[4].(*widget.Button)
			nameLabel.Show()
			speedLabel.Hide()
			totalLabel.Hide()
			p.Hide()
			...
			case 3:
				...
				nameLabel.Hide()
				if t.Info() != nil {
					totalLabel.SetText(humanize.Bytes(uint64(t.Length())))
				} else {
					totalLabel.SetText("-")
				}
			case 4:
